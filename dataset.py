...
def get_gen_mfcc_train(args, speech_dir, batch_size):
    workspace = args.workspace
    # 这里输入的干净语音和带噪语音的文件名是一样的
    # shuffle
    speech_names = os.listdir(speech_dir)
    ...
    while 1:
        for i in range(0, len(speech_names) - batch_size, batch_size):
            ...
            for speech_na in range(i, i + batch_size):
                speech_audio = read_audio(os.path.join(speech_dir, speech_names[speech_na]), 16000)
                s_audio = speech_audio[0]
                x_all, label_all = write_mfcc(s_audio, speech_names[speech_na], 50)
...
def get_test_input(args, speech_dir, speech_names):
    fs = args.sample_rate

    speech_path = os.path.join(speech_dir, speech_names)
    (speech_audio, _) = read_audio(speech_path, target_fs=fs)

    speech_x = calc_sp_by(args, speech_audio, mode='magnitude')
    clean_x = speech_x
...
def ff(args,speech_dir):
    ...
    #speech_audio = read_audio('/home3/sining/sound_class/train/4-183487-A-1.wav', 16000)[0]
        ...
        x_all, label_all = write_mfcc(speech_audio, item, 50)
        if len(x_all) != 311:
            print(item)
